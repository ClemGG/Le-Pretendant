    ...
    public class PathEditor : Editor
    {
        ...
        void DrawVertexPathInspector()
        {
            ...
            if (data.showVertexPathOptions)
            {
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    data.vertexPathMaxAngleError = EditorGUILayout.Slider(new GUIContent("Max Angle Error"), data.vertexPathMaxAngleError, 0f, 45f);
                    data.vertexPathMinVertexSpacing = EditorGUILayout.Slider(new GUIContent("Min Vertex Dst"), data.vertexPathMinVertexSpacing, 0f, 100f);
            ...
            if (data.showVertexPathDisplayOptions)
            {
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    data.vertexHandleSize = EditorGUILayout.Slider(new GUIContent("Vertex Scale"), data.vertexHandleSize, 0f, 50f);
        ...
        void DrawVertexPathSceneEditor()
        {
            ...
            //}
                ...
                for (int i = 0; i < creator.path.NumVertices; i++)
                {
                    ...
                    Handles.CubeHandleCap(0, creator.path.vertices[i], Quaternion.identity, data.vertexHandleSize * .1f, EventType.Repaint);
                }
        }
