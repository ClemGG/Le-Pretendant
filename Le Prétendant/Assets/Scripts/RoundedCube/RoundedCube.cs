using UnityEngine;
using System.Collections;
using UnityEditor;

[RequireComponent(typeof(MeshFilter), typeof(MeshRenderer))]
public class RoundedCube : MonoBehaviour
{

    public int xSize, ySize, zSize;
    public float roundness;
    public bool enableCollisions, useOffset = true;

    private Mesh mesh;
    private MeshCollider col;
    private Vector3[] vertices;
    private Vector3[] normals;
    private Color32[] cubeUV;


    [Space(10)]
    [Header("Gizmos : ")]
    [Space(10)]

    [SerializeField] bool showIDs;
    [SerializeField] bool showVertices;
    [SerializeField] bool showNormals;
    [SerializeField] float dotSize = 0.15f;



    Transform t;


    private void Awake()
    {
        Generate();
    }

    private void OnValidate()
    {
        /* 
         * Comme j'ai pas pu mettre les côtés à 1 ou la roundness à 0 
         * sans rendre le cube noir ou avoir des messages d'erreur, on va les laisser comme ça
         */

        xSize = Mathf.Max(xSize, 2);
        ySize = Mathf.Max(ySize, 2);
        zSize = Mathf.Max(zSize, 2);

        if (roundness > Mathf.Min(xSize, ySize, zSize) / 2f)
            roundness = Mathf.Min(xSize, ySize, zSize) / 2f;

        roundness = Mathf.Max(roundness, 0.0001f);

    }



    [ContextMenu("Generate Perfect Sphere")] //You can call it from the inspector
    void Generate()
    {
        GetComponent<MeshFilter>().mesh = mesh = new Mesh();
        mesh.name = "Procedural Cube";
        //To make the update work in the editor
        EditorApplication.update += EditorUpdate;


        t = transform;

        col = GetComponent<MeshCollider>();
        if (col)
            DestroyImmediate(col);

        if (enableCollisions)
        {
            col = gameObject.AddComponent<MeshCollider>();
        }


    }


    private void EditorUpdate()
    {
        if (t == null)
        {
            return;
        }

        CreateVertices();
        CreateTriangles();
        mesh.RecalculateBounds();
        mesh.RecalculateTangents();


        if (col)
        {
            col.sharedMesh = mesh;
        }
    }










    private void CreateVertices()
    {


        int cornerVertices = 8;
        int edgeVertices = (xSize + ySize + zSize - 3) * 4;
        int faceVertices = (
            (xSize - 1) * (ySize - 1) +
            (xSize - 1) * (zSize - 1) +
            (ySize - 1) * (zSize - 1)) * 2;
        vertices = new Vector3[cornerVertices + edgeVertices + faceVertices];
        normals = new Vector3[vertices.Length];
        cubeUV = new Color32[vertices.Length];

        int v = 0;
        for (int z = 0; z <= zSize; z++)
        {
            for (int x = 0; x <= xSize; x++)
            {
                SetVertex(v++, x, 0, z);
            }
            for (int y = 1; y <= ySize; y++)
            {
                SetVertex(v++, xSize, y, z);
            }
            for (int x = xSize - 1; x >= 0; x--)
            {
                SetVertex(v++, x, ySize, z);
            }
            for (int y = ySize - 1; y > 0; y--)
            {
                SetVertex(v++, 0, y, z);
            }
        }
        for (int y = 1; y < ySize; y++)
        {
            for (int x = 1; x < xSize; x++)
            {
                SetVertex(v++, x, y, zSize);
            }
        }
        for (int y = 1; y < ySize; y++)
        {
            for (int x = 1; x < xSize; x++)
            {
                SetVertex(v++, x, y, 0);
            }
        }




        mesh.triangles = null;
        mesh.vertices = vertices;
        mesh.normals = normals;
        mesh.colors32 = cubeUV;
    }

    private void SetVertex(int i, int x, int y, int z)
    {

        float ox = (Mathf.Max(xSize, ySize, zSize) == xSize) ? Mathf.Max(ySize / 2, zSize / 2) : xSize / 2;
        float oy = (Mathf.Max(xSize, ySize, zSize) == ySize) ? Mathf.Max(xSize / 2, zSize / 2) : ySize / 2;
        float oz = (Mathf.Max(xSize, ySize, zSize) == zSize) ? Mathf.Max(xSize / 2, ySize / 2) : zSize / 2;

        Vector3 offset = new Vector3(ox, oy, oz);
        Vector3 inner = vertices[i] = new Vector3(x, y, z);

        if (x < roundness)
        {
            inner.x = roundness;
        }
        else if (x > xSize - roundness)
        {
            inner.x = xSize - roundness;
        }

        if (y < roundness)
        {
            inner.y = roundness;
        }
        else if (y > ySize - roundness)
        {
            inner.y = ySize - roundness;
        }

        if (z < roundness)
        {
            inner.z = roundness;
        }
        else if (z > zSize - roundness)
        {
            inner.z = zSize - roundness;
        }


        normals[i] = (vertices[i] - inner).normalized;
        vertices[i] = inner + normals[i] * roundness;

        if (useOffset)
        {
            vertices[i] -= offset;
        }

        cubeUV[i] = new Color32((byte)x, (byte)y, (byte)z, 0);


    }
















    private void CreateTriangles()
    {
        //int quads = (xSize * ySize + xSize * zSize + ySize * zSize) * 2;
        int[] trianglesX = new int[(ySize * zSize) * 12];
        int[] trianglesY = new int[(xSize * zSize) * 12];
        int[] trianglesZ = new int[(xSize * ySize) * 12];
        int ring = (xSize + ySize) * 2;
        int tZ = 0, tX = 0, tY = 0, v = 0;
        

        for (int z = 0; z < zSize; z++, v++)
        {
            for (int x = 0; x < xSize; x++, v++)
            {
                tY = SetQuad(trianglesY, tY, v, v + 1, v + ring, v + ring + 1);
            }
            for (int y = 0; y < ySize; y++, v++)
            {
                tX = SetQuad(trianglesX, tX, v, v + 1, v + ring, v + ring + 1);
            }
            for (int x = 0; x < xSize; x++, v++)
            {
                tY = SetQuad(trianglesY, tY, v, v + 1, v + ring, v + ring + 1);
            }
            for (int y = 0; y < ySize - 1; y++, v++)
            {
                tX = SetQuad(trianglesX, tX, v, v + 1, v + ring, v + ring + 1);
            }
            tX = SetQuad(trianglesX, tX, v, v - ring + 1, v + ring, v + 1);
        }

        tZ = CreateBackFace(trianglesZ, tZ, ring);
        tZ = CreateFrontFace(trianglesZ, tZ, ring);

        mesh.subMeshCount = 3;
        mesh.SetTriangles(trianglesX, 0);
        mesh.SetTriangles(trianglesY, 1);
        mesh.SetTriangles(trianglesZ, 2);
        
    }









    private int CreateBackFace(int[] triangles, int t, int ring)
    {
        int v = 1;
        int vMid = vertices.Length - (xSize - 1) * (ySize - 1);
        t = SetQuad(triangles, t, ring - 1, vMid, 0, 1);



        for (int x = 1; x < xSize - 1; x++, v++, vMid++)
        {
            t = SetQuad(triangles, t, vMid, vMid + 1, v, v + 1);
        }
        t = SetQuad(triangles, t, vMid, v + 2, v, v + 1);



        int vMin = ring - 2;
        vMid -= xSize - 2;
        int vMax = v + 2;



        for (int y = 1; y < ySize - 1; y++, vMin--, vMid++, vMax++)
        {
            t = SetQuad(triangles, t, vMin, vMid + xSize - 1, vMin + 1, vMid);
            for (int x = 1; x < xSize - 1; x++, vMid++)
            {
                t = SetQuad(
                    triangles, t,
                    vMid + xSize - 1, vMid + xSize, vMid, vMid + 1);
            }
            t = SetQuad(triangles, t, vMid + xSize - 1, vMax + 1, vMid, vMax);
        }



        int vTop = vMin - 1;
        t = SetQuad(triangles, t, vTop + 1, vTop, vTop + 2, vMid);
        for (int x = 1; x < xSize - 1; x++, vTop--, vMid++)
        {
            t = SetQuad(triangles, t, vTop, vTop - 1, vMid, vMid + 1);
        }
        t = SetQuad(triangles, t, vTop, vTop - 1, vMid, vTop - 2);


        return t;
    }








    private int CreateFrontFace(int[] triangles, int t, int ring)
    {
        int v = ring * zSize;
        for (int x = 0; x < xSize - 1; x++, v++)
        {
            t = SetQuad(triangles, t, v, v + 1, v + ring - 1, v + ring);
        }
        t = SetQuad(triangles, t, v, v + 1, v + ring - 1, v + 2);




        int vMin = ring * (zSize + 1) - 1;
        int vMid = vMin + 1;
        int vMax = v + 2;



        for (int y = 1; y < ySize - 1; y++, vMin--, vMid++, vMax++)
        {
            t = SetQuad(triangles, t, vMin, vMid, vMin - 1, vMid + xSize - 1);
            for (int x = 1; x < xSize - 1; x++, vMid++)
            {
                t = SetQuad(
                    triangles, t,
                    vMid, vMid + 1, vMid + xSize - 1, vMid + xSize);
            }
            t = SetQuad(triangles, t, vMid, vMax, vMid + xSize - 1, vMax + 1);
        }



        int vTop = vMin - 2;
        t = SetQuad(triangles, t, vMin, vMid, vTop + 1, vTop);
        for (int x = 1; x < xSize - 1; x++, vTop--, vMid++)
        {
            t = SetQuad(triangles, t, vMid, vMid + 1, vTop, vTop - 1);
        }
        t = SetQuad(triangles, t, vMid, vTop - 2, vTop, vTop - 1);



        return t;
    }











    private static int
    SetQuad(int[] triangles, int i, int v00, int v10, int v01, int v11)
    {
        triangles[i] = v11;
        triangles[i + 1] = triangles[i + 4] = v01;
        triangles[i + 2] = triangles[i + 3] = v10;
        triangles[i + 5] = v00;
        return i + 6;
    }






    private void OnDrawGizmos()
    {
        if (vertices == null || !t)
        {
            return;
        }


        Matrix4x4 newGizmosMatrix = t.localToWorldMatrix;
        Matrix4x4 oldGizmosMatrix = Gizmos.matrix;
        Gizmos.matrix = newGizmosMatrix;

        for (int i = 0; i < vertices.Length; i++)
        {

            if (showVertices)
            {
                Gizmos.color = Color.yellow;
                Gizmos.DrawCube(vertices[i], Vector3.one * dotSize);
            }
            if (showVertices)
            {
                Gizmos.color = Color.blue;
                Gizmos.DrawCube(vertices[0], Vector3.one * dotSize);
            }
            if (showVertices)
            {
                Gizmos.color = Color.green;
                Gizmos.DrawCube(vertices[vertices.Length - 1], Vector3.one * dotSize);
            }
            if (showIDs)
            {

                Matrix4x4 newHandleMatrix = t.localToWorldMatrix;
                Matrix4x4 oldHandleMatrix = Handles.matrix;
                Handles.matrix = newHandleMatrix;


                //To draw text as Gizmo. Unfortunately, I can't change the color.
                Handles.color = Color.black;
                Handles.Label(vertices[i], i.ToString());


                Handles.matrix = oldHandleMatrix;
            }
            if (showNormals)
            {
                Gizmos.color = Color.red;
                Gizmos.DrawRay(vertices[i], normals[i]);
            }
        }

        Gizmos.matrix = oldGizmosMatrix;

    }
}